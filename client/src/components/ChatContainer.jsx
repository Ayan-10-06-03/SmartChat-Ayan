...
const ChatContainer = () => {
  ...
  const handleSummarizeChat = async () => {
    ...
    try {
      const { data } = await axios.get(`/api/messages/summary/${selectedUser._id}`);
      if (data.success) {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <p className='flex-1 text-1g text-wite flex items-center gap-2'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        ...
        {/* ✅ Chat Summary inside scrollable chat area */}
        {summary && (
          <div className="p-3 mt-4 bg-gray-900 text-white text-sm border border-gray-700 rounded-lg">
            <h4 className="font-semibold mb-2">Chat Summary</h4>
            <p>{summary}</p>
          </div>
        )}
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
      ...
      <p className='text-lg font-medium text-white'>Smart Way To Chat</p>
    </div>
  )
}
